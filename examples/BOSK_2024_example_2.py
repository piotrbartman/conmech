...
if __name__ == "__main__":
    mesh_descr = CrossMeshDescription(
        initial_position=None, max_element_perimeter=0.5 / PRECISION, scale=[1, 1]
    )
    T = 3.2
    setups = dict()
    ...
    for name, setup in setups.items():
        main(
            Config(output_dir="BOSK", force=True).init(),
            setup, name=name, steps=int(T/setups["plain"].time_step)+1
        )
